            ...
            <div className="overflow-hidden bg-gradient-to-br from-blue-100 via-indigo-100 to-purple-100 p-2 rounded-xl w-3/4">
              <div className="overflow-hidden bg-white">
                <img
                  src={project.image}
                  alt={project.title}
                  className="w-full h-[500px] md:h-[600px] object-cover object-center transition-transform duration-700"
                />
              </div>
            </div>
          </div>
                ...
                <div className="bg-white p-4 flex flex-wrap gap-3">
                  {project.tags.length > 0 ? (
                    project.tags.map((tag, i) => (
                      <span
                        key={i}
                        className="px-4 py-2 bg-gradient-to-r from-white to-blue-50 text-blue-700 rounded-full text-sm font-semibold border border-blue-200 cursor-default"
                      >
                        {tag}
                      </span>
                    ))
          ...
          <div className="flex justify-between w-3/4 mx-auto mt-16">
            ...
            {!isLast ? (
              <Link
                ...
                className="px-6 py-3 bg-gray-800 text-white rounded-full flex items-center gap-2 hover:bg-gray-900 transition-all duration-300"
              >
